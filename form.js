...
function submitForm(event) {
  ...
  let bookForm = new Book(
    titleForm.value,
    authorForm.value,
    languageForm.value,
    publishDateForm.value,
    numPagesForm.value,
    isReadForm.checked
  );
...
function fillForm(event) {
  if (event.target.classList.value.includes('edit-button')) {
    ...
    publishDateForm.value = new Date(library[index].publishDate)
      .toISOString()
      .substr(0, 10);
...
function validateForm(fieldsForm) {
  ...
  fieldsForm.forEach((field) => {
    ...
    if (
      ...
      (field.id === 'publishDate' && field.value === '') ||
      new Date(field.value).getFullYear().toString().length > 4 ||
      ((field.id === 'author' || field.id === 'language') &&
        /[^A-Za-zÀ-ú -.]/.test(field.value))
    ) {
...
$('#formModal').on('hidden.bs.modal', function () {
  ...
  Array.from(document.getElementsByClassName('is-valid')).forEach((el) =>
    el.classList.remove('is-valid')
  );
  Array.from(document.getElementsByClassName('is-invalid')).forEach((el) =>
