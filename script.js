  ...
  library.forEach((book) => {
    ...
    for (const property in book) {
      ...
      if (book.hasOwnProperty(property)) {
        switch (property) {
          case 'publishDate':
            cellElement.innerHTML = new Date(
              book[property]
            ).toLocaleDateString();
            break;
          case 'isRead':
    ...
    fileElement.insertAdjacentHTML(
      'beforeend',
      '<i class="fa-solid fa-pen-to-square p-2 hover:scale-125" data-toggle="modal" data-target="#formModal"></i>'
    );
    fileElement.insertAdjacentHTML(
      'beforeend',
      '<i class="fa-solid fa-trash remove-button p-2 hover:scale-125" data-toggle="modal" data-target="#confirmModal"></i>'
    );
